...
description: Documentation-only NestJS Swagger and JSDoc refinements without behavior or architecture changes. Use when Codex needs to document controllers, DTOs, services, utilities, or custom decorators with @nestjs/swagger and JSDoc, including ApiOperation/ApiResponse/ApiProperty usage, multipart or wrapped response schemas, and method descriptions with params, returns, and throws. If the task includes broader implementation, refactor, provider, or module work, prefer `typescript-nestjs` instead. If the user does not specify which file, symbol, or endpoint to document, ask which target to update before editing.
---
...
Use this skill to add or improve API documentation in NestJS projects without changing behavior, module wiring, or architecture.
Prefer existing project patterns over new abstractions, keep documentation scoped to the exact target the user requested, and defer broader NestJS implementation or design work to `typescript-nestjs`.
...
- If the request requires behavioral edits, module/provider changes, or broader NestJS refactoring, stop and use `typescript-nestjs` instead of expanding this documentation task.
